% STANFITOX - Class defining the fit of a Stan model, hacked from STANFIT
%
%     obj = StanFitOx(varargin);
%
...
classdef StanFitOx < handle
   ...
   methods
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      %% Constructor      
      function self = StanFitOx(varargin)
         p = inputParser;
         p.KeepUnmatched= true;
         p.FunctionName = 'StanFitOx constructor';
         ...
         if numel(self.processes) ~= numel(self.output_file)
            error('StanFitOx:constructor:InputFormat',...
      ...
      % function set.sim(self,value)
      %   if isa(value,'mcmc')
      %     self.sim = value;
      %   else
      %     error('StanFitOx:setsim:isa',...
      %           'Value is not a valid object of type mcmc().');
      %   end
      % end
      ...
      %       end
      %    else
      %       error('StanFitOx:verbose:InputFormat','Boolean scalar expected.');
      %    end
      % end
      ...
      function out = extract(self,varargin)         
         ...
         p = inputParser;
         p.FunctionName = 'StanFitOx extract';
      ...
      function [str,tab] = print(self,varargin)
         ...
         p = inputParser;
         p.FunctionName = 'StanFitOx print';
      ...
      function bool = exit_with_data(self)
         ...
         if ~isempty(self.processes) % stan called
            ...
            elseif is_running(self) && all(isnan(self.pos_)) % not finished
               ...
               fprintf('  3) Attach a listener to the StanFitOx object.\n');
      ...
      function load_samples(self)
        % Hacked from process_exit_success by Neil Oxtoby, Nov 2017
        %
        for ks=1:self.model.chains
          [hdr,flatNames,flatSamples,pos] =  mstan.read_stan_csv(...
            self.output_file(ks).name,self.model.inc_warmup);
          [names,dims,samples] = mstan.parse_flat_samples(flatNames,flatSamples);
          % Account for thinning
          if self.model.inc_warmup
            exp_warmup = ceil(self.model.warmup/self.model.thin);
          ...
          % Append to mcmc object
          self.sim_.append(samples,names,exp_warmup,exp_iter,ks);
          self.sim_.user_data{ks} = hdr;
        end
      end
